...
部署最新版 Worker 即可，无需修改 `wrangler.toml`。私有 Git 仅适用于你自己控制并信任的 Worker。使用只授予目标仓库 `Contents: Read-only` 的短期 fine-grained PAT，只把用户名写进代理 URL，在 Git 的密码提示中输入 PAT；不要把 PAT 写入 URL、配置文件、脚本或日志。
...
Deploy the latest Worker; no `wrangler.toml` change is required. Private Git support is intended only for a Worker deployment you control and trust. Use a short-lived fine-grained PAT restricted to `Contents: Read-only` for the target repository. Put only the username in the proxy URL and enter the PAT at Git's password prompt. Never put the PAT in a URL, configuration file, script, or log.
